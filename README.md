[![Build Status](https://img.shields.io/github/actions/workflow/status/gladly-team/build_tools/node.js.yml?branch=v1.x)](https://github.com/gladly-team/build_tools/actions/workflows/node.js.yml?query=branch%3Av1.x+)
[![codecov](https://codecov.io/gh/gladly-team/build_tools/branch/v1.x/graph/badge.svg)](https://app.codecov.io/gh/gladly-team/build_tools/branch/v1.x)
[![npm](https://img.shields.io/npm/v/build_tools/latest)](https://www.npmjs.com/package/build_tools)
...
[See a live demo](https://nfa-example-git-v1x-gladly-team.vercel.app/) of the [example app](https://github.com/gladly-team/build_tools/tree/main/example).
...
**If your app needs a generalized authentication solution**—not specifically Firebase authentication—you could consider using [NextAuth.js](https://github.com/nextauthjs/next-auth). NextAuth.js does *not* use Firebase authentication but supports a wide variety of identity providers, including Google. [Read more here](https://github.com/gladly-team/build_tools/discussions/522#discussioncomment-3336440) about the differences between `build_tools` and NextAuth.js to see which works best for your needs.
...
`yarn add build_tools` or `npm i build_tools`

Make sure peer dependencies are also installed:

`yarn add firebase firebase-admin next react react-dom`

> ⚠️ Note: we recommend pinning `firebase` to 9.16.0 until issue [#614](https://github.com/gladly-team/build_tools/issues/614) is resolved

Create a module to initialize `build_tools`.
...
Initialize `build_tools` in `_app.js`:
...
```js
// ./pages/api/login
import { setAuthCookies } from 'build_tools'
import initAuth from '../../initAuth' // the module you created above
...
```js
// ./pages/api/logout
import { unsetAuthCookies } from 'build_tools'
import initAuth from '../../initAuth' // the module you created above
...
Initializes `build_tools`, taking a [config](#config) object. **Must be called** before calling any other method.
...
```jsx
import { withUser, AuthAction } from 'build_tools'
...
```jsx
import { useUser, withUser } from 'build_tools'
...
See the [default handler](https://github.com/gladly-team/build_tools/blob/fda3fe1f1b69a989da8608cc30412f39c0cbe1ad/src/useFirebaseUser.js#L9) for guidance.
...
Configuration passed to `firebase-admin`'s [`initializeApp`](https://firebase.google.com/docs/admin/setup#initialize-sdk). It should contain a `credential` property (a plain object) and a `databaseURL` property. **Required** unless you initialize `firebase-admin` yourself before initializing `build_tools`.
...
Configuration passed to the Firebase JS SDK's [`initializeApp`](https://firebase.google.com/docs/reference/node/firebase#initializeapp). The `firebaseClientInitConfig.apiKey` value is **always required**. Other properties are required unless you initialize the `firebase` app yourself before initializing `build_tools` (or, less commonly, if you're running `build_tools` solely on the server side).
...
This library will **not** throw when it cannot verify an ID token. Instead, it will provide an unauthenticated user to the app. It will typically handle common auth-related errors such as `auth/id-token-expired` and `auth/user-disabled` without throwing. See [#366](https://github.com/gladly-team/build_tools/issues/366) and [#174](https://github.com/gladly-team/build_tools/issues/174) for additional background.
...
This library will **not** throw when it cannot refresh an ID token. Instead, it will provide an unauthenticated user to the app. See [#366](https://github.com/gladly-team/build_tools/issues/366) and [#174](https://github.com/gladly-team/build_tools/issues/174) for additional background.
...
_Stuck? Search [discussions](https://github.com/gladly-team/build_tools/discussions) or open your own Q&A discussion describing what you've already tried._
...
3. Try the [example app](https://github.com/gladly-team/build_tools/tree/v1.x/example) with your own Firebase credentials.
4. Read through other troubleshooting tips below!

#### I get the error "[Some setting] should not be available on the client side."

We expect certain sensitive config values to be falsy on the client side (see the [config validation code](https://github.com/gladly-team/build_tools/blob/main/src/config.js)). This is a precaution to make sure developers aren't accidentally bundling something like their Firebase private key with client JS.
...
To fix this, confirm the auth cookies are set in your browser's dev tools. If they're not set, please check that the `secure`, `sameSite`, and `path` options passed in the `build_tools` config make sense for your environment. For example, if you're testing on non-HTTPS localhost, make sure `secure` is false.
...
You can try setting up your credentials in [the example app](https://github.com/gladly-team/build_tools/tree/main/example) to be sure your app code isn't a problem.
...
See [adding a private key to Vercel](#adding-a-private-key-to-Vercel) and [this discussion](https://github.com/gladly-team/build_tools/discussions/95) on private key formatting.
...
- **Setting a single auth cookie:** This package currently sets more than one cookie to store authentication state. It's not currently possible to use a single cookie with a customized name: [#190](https://github.com/gladly-team/build_tools/issues/190)

We'd love to hear your feedback on these or other features. Please feel free to [open a discussion](https://github.com/gladly-team/build_tools/discussions)!
